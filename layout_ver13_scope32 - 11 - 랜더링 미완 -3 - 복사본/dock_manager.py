...
class DockManager:
    ...
    def _create_plot_widget(self, title, unit, min_val, max_val, color):
        ...
        # OpenGL 대신 안전한 렌더링 설정 (20251031 둘다 #처리 갑자기 느려짐 발생)
        ...
        plot_line = plot.plot(pen=pg.mkPen(color=color, width=2),antialias=True)
        self.plot_lines.append(plot_line)
        ##########
    ...
    # def _create_plot_widget(self, title, unit, min_val, max_val, color):
        ...
        # # ✅ Y축만 자동 범위 활성화 (X축은 수동 제어)
        # plot.getPlotItem().enableAutoRange(axis='y')
        # plot.getPlotItem().setAutoVisible(y=True)
        
        # # ✅ X축 자동 범위는 비활성화 (초기 상태)
        # plot.getPlotItem().disableAutoRange(axis='x')
        ...
        # def auto_range_on_double_click(event):
            # if event.double():
                # # ✅ X축, Y축 모두 자동 범위로 설정
                # plot.getPlotItem().enableAutoRange(axis='x')
                # plot.getPlotItem().enableAutoRange(axis='y')
