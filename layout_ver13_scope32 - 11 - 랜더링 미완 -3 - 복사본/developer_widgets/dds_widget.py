...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QDoubleSpinBox, QMessageBox, QGridLayout  # ← 추가
)
...
class DDSWidget(QGroupBox):
    ...
    def init_ui(self):
        ...
        # Ch0 Amp Gain
        grid_layout.addWidget(QLabel("Ch0 Amp Gain:"), row, 0)
        self.ch0_gain_spin = SmartSpinBox()
        self.ch0_gain_spin.setRange(0, 4095)
        self.ch0_gain_spin.setValue(1024)
        grid_layout.addWidget(self.ch0_gain_spin, row, 1)
        row += 1
        
        # Ch1 Amp Gain
        grid_layout.addWidget(QLabel("Ch1 Amp Gain:"), row, 0)
        self.ch1_gain_spin = SmartSpinBox()
        self.ch1_gain_spin.setRange(0, 4095)
        self.ch1_gain_spin.setValue(1024)
        grid_layout.addWidget(self.ch1_gain_spin, row, 1)
        row += 1
        
        # Ch0 Phase Offset (CEX)
        grid_layout.addWidget(QLabel("Ch0 Phase Offset (CEX):"), row, 0)
        self.ch0_phase_spin = SmartDoubleSpinBox()
        ...
        grid_layout.addWidget(self.ch0_phase_spin, row, 1)
        row += 1
        
        # Ch1 Phase Offset (RF)
        grid_layout.addWidget(QLabel("Ch1 Phase Offset (RF):"), row, 0)
        self.ch1_phase_spin = SmartDoubleSpinBox()
        ...
        grid_layout.addWidget(self.ch1_phase_spin, row, 1)
        row += 1
        
        # Set Auto RF Offset
        grid_layout.addWidget(QLabel("Set Auto RF Offset:"), row, 0)
        self.auto_offset_spin = SmartSpinBox()
        self.auto_offset_spin.setRange(0, 65535)
        self.auto_offset_spin.setValue(0)
        grid_layout.addWidget(self.auto_offset_spin, row, 1)
        
        grid_layout.setColumnStretch(2, 1)  # 3열 stretch
