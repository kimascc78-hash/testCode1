...
from PyQt5.QtWidgets import (
    ...
    QComboBox, QSpinBox, QMessageBox, QGridLayout  # ← 추가
)
...
class FastAcqWidget(QGroupBox):
    ...
    def init_ui(self):
        ...
        # Memory Type
        grid_layout.addWidget(QLabel("Memory Type:"), row, 0)
        self.memory_type_combo = QComboBox()
        self.memory_type_combo.addItems(["Ring Buffer", "Single Shot"])
        grid_layout.addWidget(self.memory_type_combo, row, 1)
        row += 1
        
        # Trigger Source
        grid_layout.addWidget(QLabel("Trigger Source:"), row, 0)
        self.trigger_source_combo = QComboBox()
        self.trigger_source_combo.addItems(["Manual", "External", "Auto"])
        grid_layout.addWidget(self.trigger_source_combo, row, 1)
        row += 1
        
        # Trigger Position
        grid_layout.addWidget(QLabel("Trigger Position:"), row, 0)
        self.trigger_position_combo = QComboBox()
        self.trigger_position_combo.addItems(["Start", "Center", "End"])
        grid_layout.addWidget(self.trigger_position_combo, row, 1)
        row += 1
        
        # Control
        grid_layout.addWidget(QLabel("Control:"), row, 0)
        self.control_combo = QComboBox()
        self.control_combo.addItems(["Stop", "Start", "Single"])
        grid_layout.addWidget(self.control_combo, row, 1)
        row += 1
        
        # Sample Rate
        grid_layout.addWidget(QLabel("Sample Rate:"), row, 0)
        self.sample_rate_spin = SmartSpinBox()
        self.sample_rate_spin.setRange(1000, 1000000)
        self.sample_rate_spin.setValue(10000)
        self.sample_rate_spin.setSuffix(" Hz")
        grid_layout.addWidget(self.sample_rate_spin, row, 1)
