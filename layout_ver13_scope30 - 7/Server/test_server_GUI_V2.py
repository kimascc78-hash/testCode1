...
class RealisticTestScenarioManager:
    ...
    def __init__(self):
        ...
        self.noise_level = 0.02  # 기본 노이즈 레벨 2%
    ...
    def normal_operation(self, status, t, client_state):
        ...
        if not client_state["rf_enabled"]:
            ...
            if set_power > 0:
                # 파워 제어 루프 시뮬레이션
                target_power = set_power
                if client_key not in self.last_values:
                    self.last_values[client_key] = {"forward_power": 0.0}
                ...
                # 현실적인 노이즈 추가
                if self.test_mode == "sine_wave":
                    power_variation = 0.05 * np.sin(2 * np.pi * 0.1 * t)
                    status["forward_power"] = set_power * (1.0 + power_variation)
                else:
                    status["forward_power"] = self.add_realistic_noise(regulated_power, 0.01)
                ...
                if self.test_mode == "sine_wave":
                    ...
                    status["reflect_power"] = self.add_realistic_noise(
                        status["forward_power"] * base_reflect_ratio, 0.05
                    )
        ...
        if self.test_mode == "sine_wave":
            ...
            status["temperature"] = self.add_realistic_noise(base_temp, 0.01)
        
        status["led_state"] = 0x0021 if client_state["rf_enabled"] else 0x0001
        status["alarm_state"] = 0x0000
        return "정상 운영 (" + ("사인파" if self.test_mode == "sine_wave" else "현실적 노이즈") + ")"
...
class RFServerGUI:
    ...
    def create_test_mode_controls(self):
        ...
        # 화이트 노이즈
        ttk.Label(self.noise_control_frame, text="화이트:", font=("Arial", 8)).grid(row=0, column=0, sticky="w", padx=(0, 2))
        ...
        white_spin = ttk.Spinbox(self.noise_control_frame, from_=0.0, to=10.0, increment=0.01,
                                textvariable=self.white_noise_var, width=6, font=("Arial", 8))
        ...
        ttk.Label(self.noise_control_frame, text="%", font=("Arial", 8)).grid(row=0, column=2, sticky="w", padx=(0, 8))
        
        # 1/f 노이즈
        ttk.Label(self.noise_control_frame, text="1/f:", font=("Arial", 8)).grid(row=0, column=3, sticky="w", padx=(0, 2))
        ...
        pink_spin = ttk.Spinbox(self.noise_control_frame, from_=0.0, to=10.0, increment=0.01,
                               textvariable=self.pink_noise_var, width=6, font=("Arial", 8))
        ...
        ttk.Label(self.noise_control_frame, text="%", font=("Arial", 8)).grid(row=0, column=5, sticky="w", padx=(0, 8))
        
        # 스파이크 노이즈
        ttk.Label(self.noise_control_frame, text="스파이크:", font=("Arial", 8)).grid(row=0, column=6, sticky="w", padx=(0, 2))
        ...
        spike_spin = ttk.Spinbox(self.noise_control_frame, from_=0.0, to=10.0, increment=0.01,
                                textvariable=self.spike_noise_var, width=6, font=("Arial", 8))
        ...
        ttk.Label(self.noise_control_frame, text="%", font=("Arial", 8)).grid(row=0, column=8, sticky="w", padx=(0, 8))
        
        # 스파이크 확률
        ttk.Label(self.noise_control_frame, text="확률:", font=("Arial", 8)).grid(row=0, column=9, sticky="w", padx=(0, 2))
        ...
        prob_spin = ttk.Spinbox(self.noise_control_frame, from_=0.0, to=1.0, increment=0.001,
                               textvariable=self.spike_prob_var, width=6, font=("Arial", 8))
        ...
        ttk.Label(self.noise_control_frame, text="%", font=("Arial", 8)).grid(row=0, column=11, sticky="w")
