...
class UIController:
    ...
    def create_menubar(self):
        ...
        tuning_menu.addAction("튜닝 설정").triggered.connect(self.parent.show_tuning_dialog)
        menubar.addMenu(tuning_menu)
        
        # 설정 메뉴 추가
        settings_menu = QMenu("Settings", self.parent)
        settings_menu.addAction("GUI 설정").triggered.connect(self.parent.show_settings_dialog)
        menubar.addMenu(settings_menu)                      
        # 로그 메뉴 - 확장
        log_menu = QMenu("Log", self.parent)
        log_menu.addAction("엑셀 저장").triggered.connect(self.parent.save_excel)
        log_menu.addAction("로그 저장").triggered.connect(self.parent.save_log)
        log_menu.addAction("자동 저장 켜기").triggered.connect(self.parent.toggle_auto_save)
        log_menu.addSeparator()
        log_menu.addAction("로그 클리어").triggered.connect(self.parent.log_manager.clear_log)
        
        # 상태 로그 표시 토글
        self.status_log_action = log_menu.addAction("상태 조회 로그 표시")
        ...
        # 뷰 메뉴 - 오실로스코프 + 상태 모니터 추가
        view_menu = QMenu("View", self.parent)
        view_menu.addAction("오실로스코프").triggered.connect(self.parent.show_oscilloscope)
        view_menu.addAction("상태 모니터 (Ctrl+M)").triggered.connect(self.parent.show_status_monitor)  # 새로 추가
        ...
        developer_action = QAction("Developer Tools...", self.parent)
    ...
    def create_settings_panel(self, main_layout):        
        ...
        # RF 제어
        upper_layout.addWidget(QLabel("설정"))
